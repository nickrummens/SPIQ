...
parser.add_argument('--loss_weights', nargs='+', type=float, default=[1,1,1,1,1,1e3], help='Loss weights (more on DIC points)')
...
def HardBC(x, f):
    ...
    Sxy = f[:, 4] *x_mapped[:, 0] * x[:, 1] * (x_max - x[:, 0])
...
mask = jax.vmap(coordMap)(X_free)[:, 1] < R
...
Free_BC = dde.PointSetOperatorBC(X_free, 0, free_surface_balance)

bcs = [Free_BC]#,Integral_BC]
num_boundary = 0
...
results_path = [r"./forward",r"/mnt/d/phd/SPIQ/plate_hole_quarter/soft"][1]
...
losshistory, train_state = model.train(
    iterations=n_iter, display_every=log_every, callbacks=callbacks#, model_save_path=save_model_path
)
