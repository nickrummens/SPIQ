...
parser.add_argument('--n_DIC', type=int, default=10, help='Number of DIC') # n_DIC**2 points
...
def boundary_top(x, on_boundary):
    return on_boundary and dde.utils.isclose(x[1], offs_y + y_max_ROI)
...
syy_bottom_bc = dde.icbc.DirichletBC(geom, lambda x: pstress, boundary_bottom, component=3)
syy_top_bc = dde.icbc.DirichletBC(geom, lambda x: pstress, boundary_bottom, component=3)
...
def HardBC(x, f, x_max=offs_x + x_max_ROI):
    ...
    Sxx = f[:, 2] * (x_max - x[:, 0]) * (x[:, 0] - offs_x)
    Syy = f[:, 3] * (x[:, 1] - offs_y) * (offs_y + y_max_ROI - x[:, 1]) + (pstress * (b+(2*indent_x))/b)
    Sxy = f[:, 4] * (x[:, 0] - offs_x) * (x_max - x[:, 0])
...
if net_type == "spinn":
    ...
    X_plot = [np.linspace(offs_x, offs_x + x_max_ROI, 100).reshape(-1,1)] * 2
    ...
    num_params = get_num_params(net, input_shape=layers[0])
    X_mesh = np.meshgrid(
        np.linspace(offs_x, 1, offs_x + x_max_ROI, dtype=np.float32),
        np.linspace(offs_y, 1, offs_y + y_max_ROI, dtype=np.float32),
        indexing="ij",
    )
